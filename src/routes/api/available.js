...
module.exports.register = async server => {
    ...
    server.route( {
        ...
        handler: async request => {
            try {
                const db = request.server.plugins.sql.client
                const {cod_house, min_hour_available, max_hour_available, day_week_available, cad_dta_available} = request.payload
    ...
    server.route( {
        ...
        handler: async (request, h) => {
            try {
                ...
                const db = request.server.plugins.sql.client
                const { min_hour_available, max_hour_available, cad_dta_available} = request.payload
